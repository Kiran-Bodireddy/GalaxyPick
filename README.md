...
cd backend
python3 -m venv .venv
./.venv/bin/pip install fastapi uvicorn motor pymongo python-dotenv httpx google-genai pydantic
./.venv/bin/pip install pytest pytest-xdist        # tests only
```

> **Don't use `requirements.txt`.** It is stale Emergent-era scaffolding: the pins fail to
> resolve on modern Python and drag in a `litellm` wheel from an unreliable host, it lists
> ~20 packages the app never imports (boto3, passlib, pandas, four auth libraries for an
> app with no auth), and it omits `httpx`, which the code does import. The line above is
...
- **Most dependencies are unused.** 45 of 46 shadcn/ui components are dead code — they
  pull in all 27 Radix packages for one toast. `framer-motion`, `recharts`, `zod`, `swr`,
  `lodash`, `date-fns` and `dayjs` are never imported.
